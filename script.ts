...
const fetchStatus = document.getElementById('fetchStatus') as HTMLDivElement
...
async function findAddressForParams(userAddress: string): Promise<string> {
    try {
...
async function fetchUserBorrowed(userAddress: string): Promise<number> {
    try {
...
// Fetch interest rate
async function fetchInterestRate(positionAddress: string): Promise<number> {
    try {
        const response = await fetch('https://lb-fullnode-alephium.notrustverify.ch/contracts/call-contract', {
            method: 'POST',
            headers: {
...
function showFetchStatus(message: string, type: 'success' | 'error' | 'info' = 'info'): void {
...
async function fetchLoanData(address: string): Promise<void> {
    if (!address.trim()) {
    ...
    try {
        ...
        if (typeof collateralData.currentCollateral === 'number') {
            ...
            const interestRateSelect = document.getElementById('interestRateSelect') as HTMLSelectElement;
